...
public class ContainerLovoFurnace extends Container{
	...
	public ContainerLovoFurnace(IInventory tile,InventoryPlayer player) {
		...
		for (int i = 0; i < 3; ++i)
        {
            for (int j = 0; j < 3; ++j)
            {
            	super.addSlotToContainer(new Slot(tile, j + i * 3 + 4, 53 + j * 18, 17 + i * 18));
            }
        }
		...
		for (int i = 0; i < 3; ++i)
        {
            for (int j = 0; j < 9; ++j)
            {
            	super.addSlotToContainer(new Slot(player, j + i * 9 + 9, 8 + j * 18, 84 + i * 18));
            }
        }

        for (int k = 0; k < 9; ++k)
        {
        	super.addSlotToContainer(new Slot(player, k, 8 + k * 18, 142));
        }
	}
